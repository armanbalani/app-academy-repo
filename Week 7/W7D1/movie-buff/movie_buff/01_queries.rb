...
def harrison_ford
  ...
  Movie.select("movies.id, title").joins(:actors).where("name = ? AND castings.ord != 1", "Harrison Ford")
...
def biggest_cast
  ...
  Movie.select("movies.id, title").joins(:castings).group("movies.id").order("COUNT(movies.id) DESC").limit(3)
...
def directed_by_one_of(them)
  ...
  Movie.select("movies.id, title").joins(:director).where("name IN (?)", them)
end
